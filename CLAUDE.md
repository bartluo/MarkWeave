...
MarkWeave is a WYSIWYG markdown editor built on Electron + Vue 3. It supports CommonMark, GitHub Flavored Markdown, math (KaTeX), Mermaid diagrams, PlantUML, and multiple editing modes (focus, typewriter, source-code).

- **Version**: see `package.json`
- **License**: MIT
- **Repository**: https://github.com/markweave/markweave
...
<repo-root>/
  package.json              Workspace orchestrator — every CI-facing script
                            ...
                            markweave ...`. CI invocations are unchanged.
  ...
  packages/
    desktop/                The Electron app (name: "markweave").
...
desktop-specific script to `packages/desktop` via `pnpm --filter markweave`,
...
`packages/website/content/docs/dev/` contains the deeper developer documentation referenced by this guide. Same files are published as the developer docs section on https://markweave.me/docs/dev/overview:
