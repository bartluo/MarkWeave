...
MarkWeave is the new name for the project formerly known as MarkText. We'd like to thank all contributors and users that have been using the app and worked on this release.
...
- MarkWeave is now single-instance application on Linux and Windows too.
...
- MarkWeave can use a default directory that is automatically opened during startup (#711)
...
- Support maxOS `dark mode`, when you change `mode dark or light` in system, MarkWeave will change its theme.
...
- Restore default MarkWeave style after exporting/printing
...
- Fixed list parse error [more info](https://github.com/markweave/markweave/issues/831#issuecomment-477719256)
- Fixed source code mode tab switching
- Fixed source code mode to preview switching
- MarkWeave didn't remove highlight when I delete the markdown symbol like * or `. (#893)
...
You need uninstall the old version of MarkWeave before install version 0.10.21, because we changed the AppId when build.
...
- Add user preferences in `MarkWeave menu`, the shoutcut is `CmdorCtrl + ,`, you can set the default `theme` and `autoSave`.
- Add `autoSave` to `file menu`, the default value is in `preferences.md` which you can open in `MarkWeave menu`. #45
- Add drag and drop to open Markdown file with MarkWeave @fxha
...
- fix: prevent open image or file directly when drag and drop over MarkWeave #42
