...
[Download the AppImage](https://github.com/markweave/markweave/releases/latest) and type the following:

1. `chmod +x marktext-%version%-x86_64.AppImage`
2. `./marktext-%version%-x86_64.AppImage`
3. Now you can execute MarkWeave.
...
See [example desktop file](https://github.com/markweave/markweave/blob/develop/resources/linux/marktext.desktop).

```bash
$ curl -L https://raw.githubusercontent.com/markweave/markweave/develop/resources/linux/marktext.desktop -o $HOME/.local/share/applications/marktext.desktop
...
- MarkWeave is always integrated into desktop environment after updating

## Binary

You can download the latest `marktext-%version%.tar.gz` package from the [release page](https://github.com/markweave/markweave/releases/latest). You may need to install electron dependencies.

## Arch User Repository

MarkWeave is available on the AUR as `marktext-bin` and will automatically install the dependencies: `glibc`, `gtk3`, `nss`, `alsa-lib`, `libxss`, `cups`, `libxkbcommon`, `libxkbfile`, `mesa`, and `hicolor-icon-theme`.
