...
MarkWeave is a free, open-source Markdown editor for **Linux**, **macOS** and **Windows**. Pre-built binaries are published with every release on [GitHub releases](https://github.com/markweave/markweave/releases/latest). Pick the one that matches your platform.
...
After installing, MarkWeave registers itself as a handler for `.md`, `.markdown`, `.mmd`, `.mdown`, `.mdtxt` and `.mdtext` files.
...
Open the DMG and drag MarkWeave into your **Applications** folder. Builds are not currently notarized, so the first launch may prompt the system Gatekeeper — right-click the app and choose **Open** to accept it once.
...
MarkWeave is shipped in five Linux formats. Most users want the AppImage.
...
Arch Linux users can install MarkWeave from the AUR (`marktext-bin`).
...
```sh
git clone https://github.com/markweave/markweave.git
cd marktext
pnpm install
pnpm run build
```
...
MarkWeave checks for updates on launch (this can be disabled under **Preferences → General → Updates**). When an update is published, the app downloads it in the background and installs on next restart.
...
| Platform | How |
| --- | --- |
| Windows | **Settings → Apps**, or run the bundled `Uninstall MarkWeave.exe`. |
| macOS | Drag **MarkWeave.app** to the Trash. Optionally also remove `~/Library/Application Support/marktext`. |
...
To wipe MarkWeave's user data as well, remove its [application data directory](APPLICATION_DATA_DIRECTORY.md).
