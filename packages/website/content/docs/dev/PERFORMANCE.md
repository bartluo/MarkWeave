...
- We mainly rely on Chrome's `Performance` tab to analyse and record the performance of MarkWeave
...
- This launches a **production build** of MarkWeave that has a break-point **before** the first line of Javascript is ran
...
- Simply go to `Performance`, press `Record`, and wait for MarkWeave to launch fully, then Stop the recording
