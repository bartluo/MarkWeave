...
// `window.markweave.paths` / `window.fileUtils` at call time, all normally
...
vi.hoisted(() => {
  const w = globalThis as unknown as {
    window?: {
      ...
      markweave?: { paths: { userDataPath: string } }
  ...
  w.window.markweave ??= { paths: { userDataPath: '/userData' } }
...
// never touch `window.fileUtils`/`window.markweave`, unlike a disk theme name)
...
describe('getCssForOptions', () => {
  ...
  it('appends no theme CSS for theme:"default" (disk lookup misses) or {}', async() => {
    ...
    // reads window.markweave.paths + window.fileUtils.isFile (→ false here).
