...
declare global {
  ...
  interface PaymentAPI {
    getPlans(): Promise<PaymentPlanInfo[]>
    checkout(plan: PaymentPlanInfo, email: string, method: 'alipay' | 'wechat' | 'creem' | 'manual', customerName?: string): Promise<CheckoutOutcome>
  ...
  interface AuthAPI {
    ...
    register(req: { email: string; password: string; displayName: string }): Promise<AuthResult>
    logout(): Promise<{ ok: boolean }>
    refreshToken(): Promise<{ ok: boolean; token?: string }>
    getProfile(): Promise<UserProfile | null>
    getDevices(): Promise<DeviceList | null>
    activateDevice(name: string): Promise<{ ok: boolean; deviceId?: string }>
    deactivateDevice(deviceId: string): Promise<{ ok: boolean }>
    linkAccount(licenseKey: string): Promise<{ ok: boolean; error?: string }>
    migrateLicense(licenseKey: string): Promise<{ ok: boolean; error?: string }>
    startOAuth(provider: 'google' | 'github'): Promise<OAuthChallenge>
    oauthCallback(code: string, state: string, provider: 'google' | 'github'): Promise<AuthResult>
    onStateChanged(handler: (state: AuthStatus) => void): void
    // ── 商业功能 ─────────────────────────────────────────────────────────────────
    getSubscription(): Promise<UserSubscription | null>
    createSubscription(planId: string, billingCycle: BillingCycle, couponCode?: string): Promise<{ ok: boolean; subscriptionId?: string; planType?: string; error?: string }>
    getCoupon(code: string): Promise<{ valid: boolean; coupon?: { type: string; value: number; name: string } | null }>
    createTeam(name: string, description?: string): Promise<{ ok: boolean; teamId?: string; name?: string; error?: string }>
    getTeam(teamId: string): Promise<TeamDetail | null>
    inviteTeamMember(teamId: string, email: string, role?: 'admin' | 'member'): Promise<{ ok: boolean; inviteCode?: string; error?: string }>
    acceptTeamInvite(code: string): Promise<{ ok: boolean; teamId?: string; role?: string; error?: string }>
    getReferral(): Promise<Referral | null>
    convertReferral(code: string, email: string): Promise<{ ok: boolean; error?: string }>
