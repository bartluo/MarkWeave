...
class KeyboardLayoutMonitor extends EventEmitter {
  ...
  // delegated to `this.removeListener(channel, callback)`, which recursed into
  // this very override and blew the stack. Delegate to the base class instead.
...
export const registerKeyboardListeners = (): void => {
  ...
  ipcMain.on('mt::keybinding-debug-dump-keyboard-info', async() => {
    const dumpPath = path.join(os.tmpdir(), 'markweave_keyboard_info.json')
