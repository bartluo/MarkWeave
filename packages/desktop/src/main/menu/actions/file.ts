...
import { t } from '../../i18n'
import { getLicenseManager } from '../../license'
...
const handleResponseForSave = async(
  ...
  // on disk nevertheless but is already tracked by MarkWeave.
...
ipcMain.on('mt::format-link-click', async(e, { data, dirname }: FormatLinkPayload) => {
  ...
  if (pathname) {
    // CommonMark #503, allow percent encoded path names to open files.
