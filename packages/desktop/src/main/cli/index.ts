...
const cli = (): ParsedArgs => {
  ...
  if (process.env.NODE_ENV === 'development') {
    // Don't pass electron development arguments to MarkWeave and change user data path.
    argv = ['--user-data-dir', path.join(getPath('appData'), 'markweave-dev')]
  }

  const args = parseArgs(argv, true)
  if (args['--help']) {
    write(`Usage: markweave [commands] [path ...]
...
`)
  ...
  if (args['--version']) {
    writeLine(`MarkWeave: ${MARKTEXT_VERSION_STRING}`)
  ...
  if (!args['--user-data-dir']) {
    const portablePath = path.join(app.getAppPath(), '..', '..', 'markweave-user-data')
