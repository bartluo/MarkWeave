...
import type { LicenseState, ActivateResult } from './license'
import type { CheckoutResult, OrderInfo, PaymentPlanInfo } from './payment'
import type {
  AuthStatus,
  AuthResult,
  LoginCredentials,
...
export interface IpcInvokeChannels {
  ...
  'mt::license::activate': { args: [licenseKey: string]; ret: ActivateResult }
  'mt::license::deactivate': { args: []; ret: { ok: boolean; error?: string } }
  'mt::license::get-state': { args: []; ret: LicenseState }
  'mt::license::has-feature': { args: [feature: string]; ret: boolean }
  'mt::license::refresh': { args: []; ret: LicenseState }
  'mt::payment::checkout': { args: [plan: PaymentPlanInfo, email: string, method: import('./payment').PaymentMethod, customerName?: string]; ret: import('./payment').CheckoutOutcome }
  'mt::payment::get-order': { args: [orderId: string]; ret: OrderInfo | null }
  'mt::payment::get-plans': { args: []; ret: PaymentPlanInfo[] }
  // ---- Auth channels ----
  'mt::auth::get-status': { args: []; ret: AuthStatus }
  'mt::auth::login': { args: [creds: LoginCredentials]; ret: AuthResult }
  'mt::auth::register': { args: [req: RegisterRequest]; ret: AuthResult }
  'mt::auth::logout': { args: []; ret: { ok: boolean } }
  'mt::auth::refresh-token': { args: []; ret: { ok: boolean; token?: string } }
  'mt::auth::get-profile': { args: []; ret: UserProfile | null }
  'mt::auth::get-devices': { args: []; ret: DeviceList | null }
  'mt::auth::activate-device': { args: [deviceName: string]; ret: { ok: boolean; deviceId?: string } }
  'mt::auth::deactivate-device': { args: [deviceId: string]; ret: { ok: boolean } }
  'mt::auth::link-account': { args: [licenseKey: string]; ret: { ok: boolean; error?: string } }
  ...
  'mt::auth::get-coupon': { args: [code: string]; ret: { valid: boolean; coupon?: { type: string; value: number; name: string } | null } }
  'mt::auth::create-team': { args: [name: string, description?: string]; ret: { ok: boolean; teamId?: string; name?: string } }
  'mt::auth::get-team': { args: [teamId: string]; ret: TeamDetail | null }
  'mt::auth::invite-team-member': { args: [teamId: string, email: string, role?: 'admin' | 'member']; ret: { ok: boolean; inviteCode?: string } }
  'mt::auth::accept-team-invite': { args: [code: string]; ret: { ok: boolean; teamId?: string; role?: string } }
  'mt::auth::get-referral': { args: []; ret: Referral | null }
  'mt::auth::convert-referral': { args: [code: string, email: string]; ret: { ok: boolean } }
  'mt::auth::get-notifications': { args: []; ret: Notification[] }
...
export interface IpcMainEventChannels {
  ...
  'mt::license::state-changed': [state: LicenseState]
  'mt::auth::state-changed': [state: AuthStatus]
