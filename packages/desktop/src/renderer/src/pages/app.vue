...
import { trackSessionStart, trackSessionEnd } from '@/util/analytics'
...
// 授权变化（激活/吊销/过期）时重新应用主题，未授权 premium 主题自动回退
watch(
  () => licenseStore.features.premiumThemes,
...
onMounted(async () => {
  ...
  if (window.markweave?.initialState) {
    preferencesStore.SET_USER_PREFERENCE(window.markweave.initialState)
  }
  ...
  nextTick(() => {
    ...
    const init = window.markweave?.initialState
