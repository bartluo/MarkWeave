      ...
      <el-tabs v-model="activeName">
        ...
        >
          ...
          <cur-select
            ...
            more="https://markweave.app/docs/export-themes"
            :value="theme"
            :options="themeList"
...
const loadThemesFromDisk = async () => {
  // markweave.paths is attached to `window` at runtime by bootstrap.ts but
  // isn't part of the typed contextBridge surface. Cast through `unknown`.
  const mw = (window as unknown as { markweave?: { paths?: { userDataPath?: string } } })
    .markweave
  const userDataPath = mw?.paths?.userDataPath
