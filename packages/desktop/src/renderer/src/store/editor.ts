...
export const useEditorStore = defineStore('editor', {
  ...
  actions: {
    ...
    UPDATE_LINE_ENDING_MENU(): void {
      ...
      if (lineEnding) {
        const { windowId } = window.markweave?.env ?? { windowId: -1 }
    ...
    FORCE_CLOSE_TAB(file: IFileState): void {
      ...
      trackEditorClose(file.pathname ?? undefined)
    ...
    EXPORT({ type, content, pageOptions }: ExportPayload): void {
      ...
      trackExportStart(type as string)
    ...
    LISTEN_FOR_EXPORT_SUCCESS(): void {
      window.electron.ipcRenderer.on('mt::export-success', (_, payload) => {
        ...
        trackExportComplete(payload?.type as string ?? '', filePath ? 1 : 0)
        notice
          .notify({
