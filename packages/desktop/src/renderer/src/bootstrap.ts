...
const parseUrlArgs = (): UrlArgs => {
  ...
  // Electron always passes `udp` (see main/windows/base.ts); fall back to a
  // placeholder only for browser preview, where userDataPath is not provided.
  const userDataPath = params.get('udp') ?? 'preview'
...
const bootstrapRenderer = (): void => {
  ...
  window.markweave = markweave as unknown as Window['markweave']
