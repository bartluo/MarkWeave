...
// 免费主题（未授权用户可用），其余主题为 premium，需授权才能使用
...
const DARK_FALLBACK_THEMES = new Set([
  'catppuccin-mocha',
  'gruvbox-dark',
  'tokyo-night',
  'tokyo-night-storm',
...
// 默认安全策略：未授权时 premium 主题不可用。licenseStore.init() 会根据
// hasFeature('premiumThemes') 调 setPremiumThemesEnabled(true) 解锁。
let premiumThemesEnabled = false

export const setPremiumThemesEnabled = (enabled: boolean): void => {
  premiumThemesEnabled = enabled
}

export const isPremiumTheme = (theme: string): boolean => !FREE_THEMES.has(theme)

export const addThemeStyle = (theme: string): void => {
  // 防盗版：premium 主题未经授权时回退到对应的基础深色/浅色主题
  if (isPremiumTheme(theme) && !premiumThemesEnabled) {
    theme = DARK_FALLBACK_THEMES.has(theme) ? 'dark' : 'light'
  }
